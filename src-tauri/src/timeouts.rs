...
/// Extended timeout for long operations (10s)
/// Used during tool execution on server (WebSearch, etc.) and context compaction
pub const TIMEOUT_EXTENDED_MS: u64 = 10000;
...
/// Max idle count during tool execution (30 × 10s = 300s = 5 min)
/// Server-side tools like WebSearch and codebase analysis can take a while
pub const MAX_IDLE_TOOL_PENDING: u32 = 30;

/// Max idle count during compaction (30 × 10s = 300s = 5 min)
...
mod tests {
    ...
    #[test]
    fn test_extended_timeout_is_10s() {
        assert_eq!(TIMEOUT_EXTENDED_MS, 10000);
        assert_eq!(extended_timeout(), Duration::from_secs(10));
    }
    ...
    #[test]
    fn test_tool_pending_wait_is_5min() {
        // 30 polls × 10000ms = 300 seconds = 5 minutes
        let total_wait_ms = MAX_IDLE_TOOL_PENDING as u64 * TIMEOUT_EXTENDED_MS;
        assert_eq!(total_wait_ms, 300_000);
    }

    #[test]
    fn test_compaction_wait_is_5min() {
        // 30 polls × 10000ms = 300 seconds = 5 minutes
